...
public class Explorer3 implements IRobotController {
    ...
    public RobotData3 robotData3; // Data store for junctions
    int direction;
    ...
    public void start() {
        ...
        if ((robot.getRuns() == 0) && (pollRun == 0))
            robotData3 = new RobotData3(); //reset the data store
        ...
        while (!robot.getLocation().equals(robot.getTargetLocation()) && active) {
            ...
            robotData3.recordMode(modeselect); // Stores the mode selected
    ...
    public int junction(IRobot robot) { // Selects a random passage from the available ones. Also stores the junction (it knows it hasn't been to it before if it "explores" into it)
        ...
        if (nonwallExits(robot) == 3) {
            robotData3.recordJunction(robot);
            robotData3.printJunction(robot);
        }
    ...
    public int btcorridor(IRobot robot) { //This method deals with backtracting in a corridor.
        int btcorridorDir;
        if (robotData3.getPreviousMode() == 0) { //This if statement is for when the robot explores and ends up looping back round to an already explored junction(So the corridor just before the junction)
    ...
    public int btjunction(IRobot robot) { //This method finds the correct direction to backtrack when at a junction or crossroad, uses stored junctions.    
        int direction = 0;
        int initialheading = robotData3.getInitialHeading(robot);

        if (robotData3.getPreviousMode() == 0) { //This part is for when a robot explores next to an already fully explored junction. But it has just come from a junction. (special case)
            direction = IRobot.BEHIND;
        } else if ((robotData3.getPreviousMode() == 1) && (nonwallExits(robot) == beenbeforeExits(robot))) { // This is normal backtracking mode if it encounters a junction while backtracking (backtracking for atleast 1 move before)
    ...
    public void reset() {
        active = false;
        robotData3.resetJunctionCounter();
        robotData3.resetMoveCounter();
        pollRun = 0;
    }
...
class RobotData3 { //Start of robotData class that includes junction infomation.
    ...
    public int[] mode = new int[100000]; //Array of what mode (explore or backtrack) the robot was in
    ...
    public void printMode() { //Prints our the data in the mode array
        ...
        int modevalue = mode[movecounter - 1];
